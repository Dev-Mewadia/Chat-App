...
import assets, { messagesDummyData as messageDummyData } from '../assets/assets'
...
const ChatContainer = ({ selectedUser, setSelectedUser }) => {
  ...
  useEffect(() => {
    if (scrollEnd.current) {
      scrollEnd.current.scrollIntoView({ behavior: "smooth" }) // fixed spelling
    }
  }, [messageDummyData]) // better dependency
...
{new Date(msg.createdAt).toLocaleTimeString([], {hour: '2-digit', minute: '2-digit'})}
        ...
        <div className='absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3'>
          <div className='flex-1 flex items-center bg-gray-100/12 px-3 rounded-full'>
           ...
           <input
  type="file"
  id="image"accept="image/png, image/jpeg, image/heic, .pdf, .doc, .docx, .txt" hidden/>
